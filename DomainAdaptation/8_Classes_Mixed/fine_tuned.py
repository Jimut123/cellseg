...
DATA_DIR = 'mixed'  # 302410 images. validate accuracy: 98.8%
H, W, C = 360, 360, 3
N_LABELS = len(index)
D = 1
...
hist_json_file = 'history_mixed_xception_fine_tuned_100e.json' 
with open(hist_json_file, mode='w') as f:
    hist_df.to_json(f)

# download the model in computer for later use
model.save('classification_mixed_xception_fine_tuned_100e.h5')

from tensorflow import keras
model = keras.models.load_model('classification_mixed_xception_fine_tuned_100e.h5')
...
def cm_analysis(y_true, y_pred, labels, ymap=None, figsize=(10,10)):
    ...
    plt.savefig('confusion_matrix_mixed_xception_fine_tuned_100e.png')
    plt.savefig('confusion_matrix_mixed_xception_fine_tuned_100e.eps')
    #plt.show()

cm_analysis(y_test_list, y_pred_list, [i for i in rev_index] , ymap=rev_index, figsize=(10,10))


with open('report_mixed_xception_fine_tuned_100e.txt', 'w') as f:
